 ...
 */
typedef enum {
	ADDR_MODE_DIRECT    = 0,  /**< Last 5 digits are a memory address */
	ADDR_MODE_IMMEDIATE = 1,  /**< Last 5 digits are the data value */
	ADDR_MODE_INDEXED   = 2,  /**< Address calculated (Value + Index) */
} AddressingMode_t;
...
typedef struct {
	...
	uint64_t timerLimit;     /**< Timer interval */
	uint64_t cyclesCounter;  /**< Current cycle count */
} CPU_t;
...
typedef struct {
	uint8_t track;        /**< Target Track */
	uint8_t cylinder;     /**< Target Cylinder */
	uint8_t sector;       /**< Target Sector */
	uint8_t ioDirection;  /**< 0: Read from Disk, 1: Write to Disk */
	address memAddr;      /**< Target physical RAM address */
	uint8_t status;       /**< Result: 0=Success, 1=Error */
